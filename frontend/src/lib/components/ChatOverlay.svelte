...
<div class="pointer-events-none absolute inset-0 flex flex-col-reverse gap-2 p-3 align-bottom">
	{#each messages as message (message.id)}
		<div
			class="flex items-center drop-shadow-sm"
		...
		>
			{#if message.player.avatarUrl}
				...
				/>
			{:else}
				<div class="h-8 w-8 flex-none rounded-full bg-gray-300"></div>
			{/if}

			<div class="ml-2 text-text-normal">
				<div class="text-sm font-semibold leading-4">{message.player.name}</div>
