...
<div
	class="flex h-full w-full flex-col items-center gap-2 p-4 justify-center-safe"
	in:scale={{ duration: 1000, easing: quintInOut }}
>
	{#each fragments as fragmentGroup}
		{#each fragmentGroup as fragment}
			{#if fragment.type === 'text'}
				<span class="text-center font-serif text-xl">{fragment.value}</span>
			...
			{:else if fragment.type === 'audio'}
				...
				/>
				<svg
					class={cn('aspect-square max-w-20', {
						'animate-pulse': animateSong,
					})}
					viewBox="0 0 24 24"
					fill="none"
